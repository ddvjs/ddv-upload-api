...
module.exports = {
  ...
  run: function () {
    ...
    .then(function getPartSizeRun () {
      ...
      if (_promise instanceof Promise) {
        ...
        return Promise.reject(util.setErrorId('GET_PART_SIZE_RETURN_PROMISE', new Error('option.getPartSize does not return Promise')))
      }
    })
    ...
    .then(function getFileIdRun () {
      ...
      if (_promise instanceof Promise) {
        ...
        return Promise.reject(util.setErrorId('GET_FILE_ID_RETURN_PROMISE', new Error('option.getFileId does not return Promise')))
      }
    })
  ...
  uploadRun: function () {
    ...
    .then(function getFilePartInfoRun () {
      ...
      if (_promise instanceof Promise) {
        ...
        return Promise.reject(util.setErrorId('GET_FILE_PART_INFO_RETURN_PROMISE', new Error('option.getFilePartInfo does not return Promise')))
      }
    })
  ...
  uploadPartFile: function (partNum) {
    ...
    }).then(function getFilePartMd5Run (res) {
      Promise.resolve().then(function () {
        var _promise = self.getFilePartSign(self.useUnderline ? util.upperCaseToUnderlineByObj(res.fileInfo) : res.fileInfo)
        if (_promise instanceof Promise) {
          return _promise
        } else {
          return Promise.reject(util.setErrorId('GET_FILE_PART_SIGN_RETURN_PROMISE', new Error('option.getFilePartSign does not return Promise')))
        }
      }).then(function getFilePartMd5Check (res2) {
        return {
          fileInfo: res2,
          rawData: res.rawData
        }
      })
    }).then(function getFilePartMd5Check (res) {
      console.log('res', res)
    })
  },
